...
stable source identities, and compiler profile. Local manifest names use the
synthetic `local/<name>` compiler identity. Source identities use
`<compiler-package>:<project-relative-path>` and never contain absolute paths.
Single-package Slice-1 plans contain no dependency edges.
...
hash, and binary hash or `artifact: null` for a library. Receipts contain only
relative managed paths and no project, compiler, runtime, or environment paths.
