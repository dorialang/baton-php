...
final class FetchCommand extends BatonCommand
{
    ...
    protected function handle(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($environment->workspace !== null) {
            $operations->fetchWorkspace($environment->workspace, DependencyOptions::network($input), $packages);
        } elseif (!$manifest instanceof WorkspaceManifest) {
            $operations->fetch($root, $manifest, DependencyOptions::network($input), $packages);
        }
